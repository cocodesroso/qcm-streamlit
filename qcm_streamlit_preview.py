...
st.set_page_config(page_title="Générateur de QCM Pronote", page_icon="🧪", layout="wide")
...
if questions:
    with st.expander("🔍 Aperçu du QCM analysé", expanded=True):
        for i, q in enumerate(questions, 1):
            st.markdown(f"**{i}. {q['texte']}**")
            for rep in q["reponses"]:
                mark = "✅" if rep in q["bonnes_reponses"] else "⬜"
                st.write(f"{mark} {rep}")

# Génération du XML
if st.button("📤 Générer le XML"):
